...
if($_POST){
    ...
    && isset($_POST['select']) && !empty($_POST['select'])){
        ...
        $creat=date('Y-m-d');
            ...
            $sql = 'INSERT INTO products (name, photo, price, description, date_ajout, category_id) VALUES (:name, :photo, :price, :description, :creat, :categ)';
            $query = $db->prepare($sql);
            $query->bindParam(':name', $name, PDO::PARAM_STR);
            $query->bindParam(':photo', $photo, PDO::PARAM_STR);
            $query->bindParam(':price', $price, PDO::PARAM_STR);
            $query->bindParam(':description', $description, PDO::PARAM_STR);
            $query->bindParam(':creat', $creat, PDO::PARAM_STR);
            ...
            try {
                ...
                $_SESSION['erreur'] = "Erreur lors de l'ajout : " . $e->getMessage();
        }
        // }else{
        //     $_SESSION['erreur'] = "Aucune catégorie trouvée pour ce produit";
...
<div class="mx-auto sm:ml-64">
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            <div class="container mx-auto">
                <h1 class="text-3xl font-bold mb-4 text-center">Ajouter un produit</h1>
                ...
                <form action="" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow-md">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="file" name="photo" id="photo" class="w-full border rounded px-4 py-2" accept="image/*">
                    </div>
                    <div class="mb-4">
                        <label for="price" class="block text-gray-700 font-bold mb-2">Prix</label>
                        <input type="number" step="0.01" name="price" id="price" class="w-full border rounded px-4 py-2" required>
                    </div>
                    <div class="mb-4">
                        <label for="description" class="block text-gray-700 font-bold mb-2">Description</label>
                        <textarea name="description" id="description" class="w-full border rounded px-4 py-2" required></textarea>
                    </div>
                    <div class="mb-4">
                        <label for="category_id" class="block text-gray-700 font-bold mb-2">Catégorie</label>
                       <?php 
                           
                           include_once("input_categ.php");
                        ?>
