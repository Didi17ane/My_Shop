...
// $limit = 2;
$query = "SELECT count(*) as nb_cat FROM categories";
$s = $db->prepare($query);
$s->execute();
$resul=$s->fetch();
$nb_cat = (int) $resul['nb_cat'];

$limit=5;
$pages= ceil($nb_cat/$limit);
$page1= ($pageAc * $limit) - $limit;


if($_GET['recherche'])
{
   $cat_id=strip_tags($_GET['recherche']);
  ...
  if($rech)
  {
     ...
     $_SESSION['erreur'] = "Cette categorie n'existe pas";
  }
}else{
   $sql = "SELECT * FROM categories ORDER BY id ASC LIMIT :page1, :lim";
   $q = $db->prepare($sql);
...
<div class="mx-auto sm:ml-64">
    ...
    <h1 class="text-3xl font-bold mb-3 text-center">Liste des catégories</h1>
...
<form class="m-2 shadow-md h-10 flex space-x-4">
    ...
    <div><button class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 border border-green-700 rounded"><a href="Ajouter_category.php">NewCategory</a></button></div>
</form>
...
<table class="w-full bg-white text-center">
    <thead>
        <tr class="bg-gray-200">
            ...
            <th class="px-4 py-2">Catégorie parente</th>
        ...
        <?php foreach ($cat as $category): ?>
            <tr class="border-b">
                ...
                <?php 
                     ...
                     $quer = $db->prepare($sql);
                     $quer->bindParam(':id_cat', $category['parent_id'], PDO::PARAM_INT);
                     $quer->execute();
                     $parent = $quer->fetch();
                ?>
                <td class="px-4 py-2"><?= htmlspecialchars($parent['name']) ?></td>
                <td class="px-4 py-2">
                    <a href="Modification_category.php?id=<?= $category['id'] ?>" class="text-blue-500 hover:underline">Modifier</a>
                    </td>
            </tr>
        <?php endforeach; ?>
    </tbody>
