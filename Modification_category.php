...
if(!$cat) {
    $_SESSION['erreur'] = "Catégorie inexistante";
...
if($_POST){
    ...
    && isset($_POST['select']) && !empty($_POST['select'])){
        ...
        $name = strip_tags($_POST['name']);
            $sql = 'UPDATE categories SET name=:name, parent_id=:? WHERE id=:cat_id';
            $query = $db->prepare($sql);
            $query->bindParam(':cat_id', $cat_id, PDO::PARAM_INT);
            $query->bindParam(':name', $name, PDO::PARAM_STR);
            $query->bindParam(':?', $_POST['select'], PDO::PARAM_STR);
...
<div class="mx-auto sm:ml-64">
    <div class="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700"> 
            <div class="container mx-auto p-6">
                <h1 class="text-3xl font-bold mb-4 text-center">Modifier une catégorie</h1>
                ...
                <form action="" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded shadow-md">
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" id="name" value="<?= htmlspecialchars($cat['name']) ?>" class="w-full border rounded px-4 py-2" required>
                    </div>
                    <div class="mb-4">
                       <label for="category_id" class="block text-gray-700 font-bold mb-2">Catégorie parente</label>
                        <?php  include_once("ajout_categ.php")?>
                    </div>
