...
if($_GET['rech'])
{
   $id= strip_tags($_GET['rech']);
   $connexion->setId($id);
   $rech=$connexion->Search();
   if($rech)
   {
      $all=$connexion->ShowUser();
   }else{
...
if (isset($_GET['pag']) && !empty($_GET['pag'])) {
   $pagAc = (int) strip_tags($_GET['pag']); 
} else{
   $pagAc = 1;
}

$que = "SELECT count(*) as nb_user FROM users";
$sb = $db->prepare($que);
$sb->execute();
$resulta=$sb->fetch();
$nb_user = (int) $resulta['nb_user'];

   // $nbrU=Pagi('users');
   $limit=2;
   $pags= ceil($nb_user/$limit);
   $pag1= ($pagAc * $limit) - $limit;
   $sql = "SELECT * FROM users ORDER BY id DESC LIMIT :page1, :lim";
    $q = $db->prepare($sql);
    $q->bindParam(':page1', $pag1, PDO::PARAM_INT);
    $q->bindParam(':lim', $limit, PDO::PARAM_INT);
    $q->execute();
    $all = $q->fetchAll(PDO::FETCH_ASSOC);
   // $all= Pagination('users', 'id', $page1, $limit);
}
...
$sql = "SELECT * FROM products ORDER BY cod_prod DESC LIMIT :page1, :lim";
$q = $db->prepare($sql);
...
{ ?>
    ...
    <div class="border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 sm:auto mx-100%">
                    <h1 class="text-3xl font-bold mb-3 text-center">Liste des Utilisateurs</h1>
                    ...
                    <form class="m-2 shadow-md h-10">
                        <input name="rech" class="p-2 w-full" type="text" placeholder="Rechercher...">
                    </form>
                        <table class="w-full bg-white text-center">
                              <thead>
                                 <tr class="bg-gray-200">
                                    ...
                                    <!-- <th class="px-4 py-2 hidden sm:flex">Password</th> -->
                              ...
                              <tbody>
                                 <?php foreach ($all as $val): ?>
                                    <tr class="border-b">
                                          <td class="px-4 py-2"><?= ($val['id']) ?></td>
                                          <td class="px-4 py-2"><?= htmlspecialchars($val['username']) ?></td>
                                          <td class="px-4 py-2 hidden sm:block"><?= htmlspecialchars($val['email']) ?></td>
                                          <!-- <td class="px-4 py-2 hidden sm:flex"><?= htmlspecialchars($val['password']) ?></td> -->
                                          <td class="px-4 py-2 <?php echo ($val['admin']==1) ?"font-bold text-green-500": "font-bold text-red-500"?>"><?= (htmlspecialchars($val['admin'])==1)? "Admin" : "User" ?></td>
                                       <td class="px-4 py-2"><?= htmlspecialchars($val['date_creat']) ?></td>
                                          <td class="px-4 py-2">
                                             <a href="ModifyUser.php?id=<?=$val['id']?>" class="text-blue-500 hover:underline">Modifier</a> |
                                             <a href="DeleteUser.php?id=<?=$val['id'] ?>&req=delete" class="text-red-500 hover:underline" onclick="return confirm('Êtes-vous sûr ?')">Supprimer</a>
                                          </td>
                                    </tr>
                                 <?php endforeach; ?>
                              </tbody>
